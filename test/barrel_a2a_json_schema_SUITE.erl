...
%%% @end
%%%-------------------------------------------------------------------
-module(barrel_a2a_json_schema_SUITE).
...
no_network_dereference(_Config) ->
    ...
    ?assertMatch({error, {unresolved_ref, _}}, barrel_a2a_jsonschema:compile(Schema)),
    ?assertNot(barrel_a2a_jsonschema:is_valid_schema(Schema)),
    %% Supplied through the registry, the same reference resolves.
    Registry = #{
        <<"https://example.com/not-supplied.json">> => #{<<"type">> => <<"string">>}
    },
    {ok, Compiled} = barrel_a2a_jsonschema:compile(Schema, Registry),
    ?assertEqual(ok, barrel_a2a_jsonschema:validate(<<"x">>, Compiled, #{})),
    ?assertMatch({error, _}, barrel_a2a_jsonschema:validate(1, Compiled, #{})).
...
every_reference_form_resolves(_Config) ->
    ...
    lists:foreach(
        fun(Ref) ->
            ...
            ?assertEqual(
                ok,
                barrel_a2a_jsonschema:validate(<<"x">>, Compiled, #{}),
                Ref
            ),
            ?assertMatch(
                {error, _},
                barrel_a2a_jsonschema:validate(1, Compiled, #{}),
                Ref
            )
        end,
        Refs
...
bounds_are_enforced(_Config) ->
    ...
    ?assertMatch({error, too_deep}, barrel_a2a_jsonschema:compile(Deep)).

%% The A2A bundle declares 2020-12. Validating a schema of another
%% draft under these rules would apply the wrong ones to `items',
%% `exclusiveMinimum' and `definitions', so other dialects are refused
%% with a clear error rather than misjudged.
dialects_other_than_2020_12_are_refused(_Config) ->
    ...
    Accepted = [
        ...
        %% Draft-07 means the same as 2020-12 for this document.
    ...
    lists:foreach(
        fun({Why, Dialect}) ->
            ?assertMatch({Why, {ok, _}}, {Why, barrel_a2a_jsonschema:compile(Object(Dialect))})
        end,
        Accepted
    ),
    Refused = [
    ...
    lists:foreach(
        fun({Why, Dialect}) ->
            ?assertMatch(
                ...
                {Why, barrel_a2a_jsonschema:compile(Object(Dialect))}
            )
        end,
        Refused
    ),
...
%% would quietly change what every wire object is measured against.
%% The hashes are recorded in priv/jsonschema/VENDORED.md.
vendored_metaschema_is_unchanged(_Config) ->
    Dir = filename:join(code:priv_dir(barrel_a2a), "jsonschema"),
    lists:foreach(
        fun({File, Expected}) ->
...
%% A draft-07 document using only what means the same under 2020-12
%% compiles; one using a construct 2020-12 reads differently is still
%% refused, gracefully.
draft_07_is_accepted_when_nothing_differs(_Config) ->
    ...
    {ok, C} = barrel_a2a_jsonschema:compile(Same),
    ?assertEqual(ok, barrel_a2a_jsonschema:validate(#{<<"message">> => <<"x">>}, C, #{})),
    ?assertMatch({error, _}, barrel_a2a_jsonschema:validate(#{}, C, #{})),
    Differs = [
        Same#{<<"definitions">> => #{}},
    ...
    lists:foreach(
        fun(S) ->
            ?assertMatch({error, {unsupported_dialect, _}}, barrel_a2a_jsonschema:compile(S))
        end,
        Differs
    ).
...
run_group(File, Group, Remotes) ->
    ...
    case barrel_a2a_jsonschema:compile(Schema, Remotes) of
        {error, Reason} ->
...
run_test(Compiled, Test) ->
    ...
    Actual =
        case barrel_a2a_jsonschema:validate(Data, Compiled, #{}) of
            ok -> true;
            {error, _} -> false
        end,
...
suite_dir() ->
    filename:join([code:lib_dir(barrel_a2a), "..", "..", "..", "..", "test", "json_schema_suite"]).
